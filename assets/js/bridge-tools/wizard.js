  ...
  if (loginType === 1) {
    var result = await Swal.fire({
      ...
      html: '<div style="text-align:left;max-height:400px;overflow-y:auto;">' +
        ...
        '<p><strong>' + translateThis('Security Notice') + ':</strong> ' + translateThis('We only recommend this option if you trust the source code of this site. You may also wish to run the code locally. You are responsible for risks of direct key handling.') + '</p><br>' +
...
window.launchAutomationWizard = async function() {
  ...
  var polNeedGas = wizardState.polBalance < 10 || (wizardState.polBalance * wizardState.polPrice) < 5;
  ...
  if (choices.lido) {
    disclaimers.push('<li><strong>' + translateThis('Lido HODL') + ':</strong> ' + translateThis('By proceeding, you acknowledge that the desired ETH will be traded into Lido Staked ETH through the decentralized exchange Curve. 100% of staking yields go to BAY stakers. Your principal is locked until unlock date. Lido is well-audited but carries contract risk.') + '</li>');
  }
  if (choices.stable) {
  ...
  if (choices.bay || choices.bayr) {
    ...
    disclaimers.push('<li><strong>' + translateThis('On-Ramp') + ':</strong> ' + translateThis('This website is not affiliated with any third-party payment processor. For convenience, an automatic trading address has been set up to perform the trade into BitBay. Do not send funds to this address manually. It is only used during the automated process.') + '</li>');
  }

  var disclaimerResult = await Swal.fire({
    title: translateThis('Important Disclaimers'),
    html: '<div style="max-height:50vh;overflow-y:auto;text-align:left;font-size:0.9em;padding-right:4px;">' +
  ...
  var ethResult = await Swal.fire({
    ...
    html: '<div style="text-align:left;">' +
      ...
      '<p style="font-size:0.85em;color:#777;">' + translateThis('Current ETH price') + ': ' + ethPriceDisplay + '</p>' +
  ...
  var lidoETH = choices.lido ? remainingETH * (choices.allocLido / totalAlloc) : 0;
  var stableETH = choices.stable ? remainingETH * (choices.allocStable / totalAlloc) : 0;
